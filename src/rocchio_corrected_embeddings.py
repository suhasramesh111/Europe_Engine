...
class Rocchio_:
    ...
    def __init__(self, model):
        ...
        print("Loading Word to cluster..")
        self.word_to_cluster, self.vocab_list, self.term_vectors = self.load_word_to_cluster()
        print("Done")
    ...
    def clean_data(self, data):
        ...
        clean_tokens = [
            ...
            and not any(char.isdigit() for char in word)  # remove any words with digits
    ...
    def get_association_QE(self, query):
        print("Getting Associative Expansion..")
        start_time = time.time()
        cleaned_query       = self.clean_data(query)
        cooccurrence_matrix = load_npz(self.coomat_path)

        full_vocab_list     = list(self.vectorizer.get_feature_names_out())
        vocab_index         = {word: idx for idx, word in enumerate(full_vocab_list)}

        embed_vocab = [w for w in full_vocab_list if w.isalpha() and len(w) > 2 and w not in self.stop_words]
        embedder = self.embedder
        query_embed = embedder.encode([" ".join(word_tokenize(cleaned_query))], normalize_embeddings=True)

        expanded_terms = []

        for token in word_tokenize(cleaned_query):
            if token in vocab_index:
                ...
                for i in top_indices:
                    if i < len(full_vocab_list):
                        term = full_vocab_list[i]
                        if cooc_vector[i] >= 1 and term not in expanded_terms and term in embed_vocab:
                            candidate_terms.append(term)
                    if len(candidate_terms) >= 15:
                        break

                if candidate_terms:
                    term_embeddings = embedder.encode(candidate_terms, normalize_embeddings=True)
                    similarities = cosine_similarity(query_embed, term_embeddings)[0]
                    filtered_terms = [
                        term for sim, term in sorted(zip(similarities, candidate_terms), reverse=True)
                        if sim >= 0.3 and term not in expanded_terms
                    ]
                    expanded_terms.extend(filtered_terms[:5])
        print("[Candidate Terms] : ", expanded_terms)

        if len(expanded_terms) == 0:
            print("[Using Assoc. Backup]")
            assoc_expanded_query = self.get_association_QE_backup(query)

            return assoc_expanded_query

        final_terms = word_tokenize(query) + expanded_terms[:WORD_LIMIT]
    ...
    def load_word_to_cluster(self):
        vocab_list = list(self.vectorizer.get_feature_names_out())
        term_vectors = self.embedder.encode(vocab_list, normalize_embeddings=True)
        # query_vector = self.embedder.encode([" ".join(query_tokens)], normalize_embeddings=True)

        if QE_CLUSTERS >= len(vocab_list):
            print("Too many clusters for vocabulary size.")
            return query

        kmeans = KMeans(n_clusters=QE_CLUSTERS, random_state=42)
        labels = kmeans.fit_predict(term_vectors)
        word_to_cluster = {word: label for word, label in zip(vocab_list, labels)}

        return word_to_cluster, vocab_list, term_vectors

    def get_metric_QE(self, query, response):
        print("Getting Metric Expansion..")
        start_time = time.time()
        embedder = self.embedder
        query_tokens = word_tokenize(self.clean_data(query))
        expanded_terms = list(query_tokens)

        # vocab_list = list(self.vectorizer.get_feature_names_out())
        # term_vectors = embedder.encode(vocab_list, normalize_embeddings=True)
        query_vector = embedder.encode([" ".join(query_tokens)], normalize_embeddings=True)
        ...
        for token in query_tokens:
            if token in self.word_to_cluster:
                ...
                cluster_terms = [w for w in self.vocab_list if self.word_to_cluster[w] == cluster_id]
                cluster_vecs = embedder.encode(cluster_terms, normalize_embeddings=True)
                similarities = cosine_similarity(query_vector, cluster_vecs)[0]
                sorted_terms = [term for _, term in sorted(zip(similarities, cluster_terms), reverse=True)]
                candidate_terms.extend([t for t in sorted_terms if t not in expanded_terms])

        print("Candidates : ", candidate_terms)
        if len(candidate_terms) == 0:
            print("[Using backup Metric]")
            backup_expanded_query = self.get_metric_QE_backup(query, response)

            return backup_expanded_query

        expanded_terms.extend(candidate_terms[:WORD_LIMIT])
        expanded_query = " ".join(expanded_terms[:len(query_tokens) + WORD_LIMIT])
        print("Expanded Query:", expanded_query)
        end_time = time.time()
        print("[Time] ", end_time - start_time, 's')
        return expanded_query
    ...
    def get_scalar_QE(self, query, response):        
        ...
        print("Getting Scalar Expansion..")
        start_time = time.time()
        embedder = self.embedder
        query_tokens = word_tokenize(self.clean_data(query))
        expanded_terms = list(query_tokens)
        ...
        svd = TruncatedSVD(n_components=min(QE_CLUSTERS, self.term_vectors.shape[1]), random_state=42)
        reduced_vectors = svd.fit_transform(self.term_vectors)
        ...
        for token in query_tokens:
            if token in self.word_to_cluster:
                ...
                cluster_terms = [w for w in self.vocab_list if self.word_to_cluster[w] == cluster_id]
                cluster_vecs = embedder.encode(cluster_terms, normalize_embeddings=True)
                reduced_cluster_vecs = svd.transform(cluster_vecs)
                similarities = cosine_similarity(reduced_query_vector, reduced_cluster_vecs)[0]
                sorted_terms = [term for _, term in sorted(zip(similarities, cluster_terms), reverse=True)]
                candidate_terms.extend([t for t in sorted_terms if t not in expanded_terms])
        ...
        if len(candidate_terms) == 0:
            print("[Using backup Scalar Rocchio]")
            backup_expanded_query = self.pseudo_relevance_(query, response)
            backup_expanded_query = " ".join(backup_expanded_query)
            # backup_expanded_query = str(query) + " " + str(backup_expanded_query)
    ...
    def get_metric_QE_backup(self, query, response):
        ...
        print("Expanded Query (Metric Backup): ",expanded_query)

        return expanded_query

    def get_association_QE_backup(self, query):
...
#     # sample_query = 'Black Sea'
#     while True:
#         sample_query = input(":>")
#         sample_query = sample_query.replace("\n", "").strip()
#         if sample_query == 'q':
#             break
#         rocchio_handle.get_association_QE(sample_query)
#         rocchio_handle.get_metric_QE(sample_query)
#         rocchio_handle.get_scalar_QE(sample_query)
